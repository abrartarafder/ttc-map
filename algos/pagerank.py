...
def write_pagerank_js(
    metrics_df: pd.DataFrame,
    stop_top5: pd.DataFrame,
    station_top10: pd.DataFrame,
    output_path: Path,
) -> None:
    """Write a browser-friendly payload for the map tab."""
    stop_metrics = {}
    for row in metrics_df.itertuples(index=False):
        stop_metrics[str(int(row.stop_id))] = {
    ...
    payload = {
        "stopMetricsByStopId": stop_metrics,
        "stopTop5": json.loads(stop_top5.to_json(orient="records")),
        "stationTop10": json.loads(station_top10.to_json(orient="records")),
    }
    output_path.write_text(
        "window.TTC_PAGERANK_DATA = " + json.dumps(payload, ensure_ascii=False) + ";\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    write_pagerank_js(metrics_df, stop_top5, station_top10, OUTPUT_DIR / "pagerank_data.js")
    ...
    print(f"    {OUTPUT_DIR / 'pagerank_data.js'}")
